...
- 経営企画・営業・CS などのビジネス課題を、データモデルや業務システムへ落とし込み、要件定義から運用定着までを主導
- データ領域での実務 4 年（2022/04〜）。BigQuery + dbt を中心としたデータ基盤の設計・構築・運用が専門
- 約 20 データソース / 約 1.3 TB / 7,000 テーブル超の基盤を構築し、事業部メンバーが自走できる運用体制を確立
- 個人情報の統制設計、権限管理のコード化、定期レビューを通じてデータガバナンスを整備し、成熟度スコアを 0.5 から 3.2 へ改善
...
経営企画や営業・CS などの事業部門から課題を直接ヒアリングし、業務要件を指標・データモデル・システムへ落とし込んできました。管理会計基盤では、複数部門に分散していた算出ロジックを再設計し、実績値算出を 3 人日から 10 分へ短縮しました。技術の導入を目的とせず、利用者が継続的に活用できる運用まで含めて設計します。
...
個人情報の参照範囲を制御し、権限をコードで管理して定期的に見直す運用を構築してきました。仕組みの導入後も DMBOK に基づく成熟度アセスメントを継続し、データガバナンス領域のスコアを 0.5 から 3.2 へ改善しています。安全性と利便性を両立し、利用者が信頼してデータを活用できる状態を維持します。

---

## 今後の展望

統制と信頼性を組織全体の仕組みとして設計し、データの価値を継続的に高められるエンジニアを目指しています。これまでプロジェクト単位で進めてきたセキュリティとガバナンスを全社へ展開し、データ契約や SLO によって品質を利用者へ約束できる基盤づくりに取り組みたいと考えています。さらに、AI エージェントが安全にデータを扱える提供方法の設計にも注力していきます。
